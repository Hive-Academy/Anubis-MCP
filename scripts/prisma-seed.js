...
function getJsonBasePath() {
    ...
    throw new Error(`❌ Could not find enhanced-workflow-rules/json directory in any of: ${possiblePaths.join(', ')}`);
}
...
async function seedWorkflowRoles(jsonBasePath) {
    ...
    for (const roleName of ROLES) {
        const roleDefPath = path.join(jsonBasePath, roleName, 'role-definition.json');
    ...
    for (const roleName of ROLES) {
        ...
        for (const step of stepsData.workflowSteps) {
            try {
                ...
                if (step.approachGuidance?.stepByStep &&
                        ...
                        if (action.actionType === 'MCP_CALL') {
                            await prisma.mcpAction.create({
                                data: {
                                    ...
                                    parameters: {
                                        requiredParameters: action.actionData.requiredParameters || [],
                                    },
                ...
                if (step.conditions && step.conditions.length > 0) {
                    for (const condition of step.conditions) {
                        if (condition.conditionType === 'PREVIOUS_STEP_COMPLETED') {
                            const dependsOnStep = condition.logic?.parameters?.stepName || 'previous_step';
                ...
                console.error(`❌ Error creating step ${step.name} for role ${roleName}:`, error);
                throw error;
            }
        }
    }
...
async function seedRoleTransitions(jsonBasePath) {
    ...
    for (const roleName of ROLES) {
        const transitionsPath = path.join(jsonBasePath, roleName, 'role-transitions.json');
        ...
        for (const transition of transitionsData.roleTransitions) {
            try {
                ...
                if (!fromRole || !toRole) {
                    console.warn(`⚠️  Skipping transition ${transition.transitionName}: Role not found`);
                    continue;
                }
                ...
                if (transition.validationRules) {
                    for (const [criterion] of Object.entries(transition.validationRules)) {
                ...
                if (transition.contextPreservation) {
                    for (const [contextKey] of Object.entries(transition.contextPreservation)) {
                ...
                console.error(`❌ Error creating transition ${transition.transitionName}:`, error);
                throw error;
            }
        }
    }
...
async function validateSeeding() {
    ...
    if (roleCount === 0 || stepCount === 0) {
        throw new Error('❌ Seeding validation failed: No roles or steps were created');
    }
    console.log('✅ Seeding validation passed');
}
async function main() {
    try {
        console.log('🚀 Starting Anubis database seeding (Streamlined Schema)...\n');
